...
<?php print_r([
    ...
    'cash_cheque_knet' => $cash_cheque_knet ?? null,
    'being_of' => $being_of ?? null,
]); ?>
</pre> -->
...
<body>
    ...
    <button
        onclick="window.location.href='<?= base_url('receiptvoucher/index/' . $transaction_id) ?>'"
        style="background-color: #991b36; color: white; padding: 8px 16px; border: none; border-radius: 5px;">
        Print
    </button>
<?php endif; ?>
            ...
            <?php if (!empty($transaction_id)): ?>
    <button onclick="window.location.href='<?= base_url('receiptvoucher/edit/' . $transaction_id) ?>'"
        style="background-color: #a1263a; color: white; padding: 8px 16px; border: none; border-radius: 5px; margin-left: 10px;">
        Edit
    </button>
<?php endif; ?>
        ...
        <div class="voucher-container">
            ...
            <div class="header">
                <?php if (!empty($company['company_logo'])): ?>
                    <img src="<?= base_url('public/uploads/' . $company['company_logo']) ?>" alt="Company Logo"
                        style=" max-height: 70px; width: 35%; margin-top: 8px;">
                <?php endif; ?>
            </div>
            ...
            <div class="amount-box">
                <div class="amount-field">
                    ...
                    <div class="amount-value"><?= esc($kd) ?></div>
                </div>

                <div class="amount-field">
                    <span class="amount-label">Fils فلس</span>
                    <div class="amount-value" style="padding: 20px 33px;"><?= esc($fils) ?></div>
                </div>
            </div>
    ...
    <span class="label" style="font-size: 20px;">
        <strong>No: <?= esc($invoice_no ?? '') ?></strong>
    </span>
</div>
           <div class="voucher-meta">
    Date:
    <strong><?= esc($invoice_date ?? '') ?></strong>
    التاريخ:
</div>

            <div class="field">
                <span class="label">Received From Mr/Ms: </span>
                <span class="dots"><?= esc($customer['customer_name'] ?? '') ?></span> استلمت من السيد/الآنسة
            </div>


            <div class="field">
                <span class="label">The Sum of K.D.</span>
                <span class="dots" style="width:65%;"><?= esc($amount_in_words) ?></span> مجموع الدينار الكويتي
            </div>


            <div class="field">
                <span class="label"> Cash / Cheque No. / K-Net</span>
                <span class="dots" style=" width: 54%;"><?= esc($cash_cheque_knet) ?></span> كي-نت /رقم النقد / الشيك
            </div>
            <div class="field">
                <span class="label">Being Of:</span>
                <span class="dots" style="width: 81%;"><?= esc($being_of) ?></span>كونه من
            </div>
            ...
            <div class="bottom-footer">
                ...
                <div style="margin-top: 5px;">
                    📞 <?= esc($company['phone'] ?? '') ?> &nbsp;&nbsp; | &nbsp;&nbsp;
                    📧 <a href="mailto:<?= esc($company['email'] ?? '') ?>"
